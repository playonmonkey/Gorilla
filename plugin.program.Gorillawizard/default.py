...
def index():
	...
	if not ADDONFILE == 'http://': addDir ('Addon Installer' ,'addons', icon=ICONCONTACT, themeit=THEME1)
	...
	if DEVELOPER == 'true': addDir('Developer Menu','developer', icon=ICONCONTACT, themeit=THEME1)
	setView('files', 'viewType')
	
def backuprestore():
		if HIDESPACERS == 'No': addFile(wiz.sep('Backup'), '', themeit=THEME3)
		addFile('Back Up Location: [COLOR %s]%s[/COLOR]' % (COLOR2, MYBUILDS),'settings', 'Maintenance', icon=ICONSAVE, themeit=THEME1)
		addFile('[Back Up]: Build',               'backupbuild',     icon=ICONSAVE,   themeit=THEME1)
		addFile('[Back Up]: GuiFix',              'backupgui',       icon=ICONSAVE,   themeit=THEME1)
		addFile('[Back Up]: Theme',               'backuptheme',     icon=ICONSAVE,   themeit=THEME1)
		addFile('[Back Up]: Addon_data',          'backupaddon',     icon=ICONSAVE,   themeit=THEME1)
		if HIDESPACERS == 'No': addFile(wiz.sep('Restore'), '', themeit=THEME3)
		addFile('[Restore]: Local Build',         'restorezip',      icon=ICONSAVE,   themeit=THEME1)
		addFile('[Restore]: Local GuiFix',        'restoregui',      icon=ICONSAVE,   themeit=THEME1)
		addFile('[Restore]: Local Addon_data',    'restoreaddon',    icon=ICONSAVE,   themeit=THEME1)
		addFile('[Restore]: External Build',      'restoreextzip',   icon=ICONSAVE,   themeit=THEME1)
		addFile('[Restore]: External GuiFix',     'restoreextgui',   icon=ICONSAVE,   themeit=THEME1)
		addFile('[Restore]: External Addon_data', 'restoreextaddon', icon=ICONSAVE,   themeit=THEME1)
		if HIDESPACERS == 'No': addFile(wiz.sep('Delete All Backups'), '', themeit=THEME3)
		addFile('Clean Up Back Up Folder',        'clearbackup',     icon=ICONSAVE,   themeit=THEME1)
...
def addon():
		if HIDESPACERS == 'No': addFile(wiz.sep('Addon Tools'), '', themeit=THEME2)
...
def misc():
		...
		errorsinlog = int(kodilog) + int(kodioldlog)
		wizlogsize = ': [COLOR red]Not Found[/COLOR]' if not os.path.exists(WIZLOG) else ": [COLOR green]%s[/COLOR]" % wiz.convertSize(os.path.getsize(WIZLOG))
		if HIDESPACERS == 'No': addFile(wiz.sep('Misc Maintenance'), '', themeit=THEME2)
		...
		addFile('View Errors in Log: [COLOR red]%s[/COLOR] Error(s)' % (errorsinlog), 'viewerrorlog',    icon=ICONMAINT, themeit=THEME3)
...
def backup():
		if HIDESPACERS == 'No': addFile(wiz.sep('Backup'), '', themeit=THEME2)
		...
		if HIDESPACERS == 'No': addFile(wiz.sep('Restore'), '', themeit=THEME2)
		...
		if HIDESPACERS == 'No': addFile(wiz.sep('Delete All Backups'), '', themeit=THEME2)
		addFile('Clean Up Back Up Folder',        'clearbackup',     icon=ICONMAINT,   themeit=THEME3)
def tweaks():
		if HIDESPACERS == 'No': addFile(wiz.sep('System Tweaks/Fixes'), '', themeit=THEME2)
		addFile('Advanced Settings',      'autoadvanced',      icon=ICONMAINT, themeit=THEME3)
...
def buildMenu():
	AddPack       = wiz.workingURL(ADDONPACK)
	WORKINGURL       = wiz.workingURL(BUILDFILE)
	if not WORKINGURL == True:
		addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONCONTACT, themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addFile('Url for txt file not valid', '', icon=ICONCONTACT, themeit=THEME3)
		addFile('%s' % WORKINGURL, '', icon=ICONCONTACT, themeit=THEME3)
	else:
		...
		if len(match) == 1:
			...
			addFile('[B]%s Version: %s[/B]' % (MCNAME, KODIV), '', icon=ICONCONTACT, themeit=THEME2)
			addDir ('Save Data Menu'       ,'savedata', icon=ICONSAVE,     themeit=THEME2)
			if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
			addDir ('[COLOR yellow]---[B][COLOR lime]Addon Packs [COLOR blue]/ [COLOR red]Fixes[/COLOR][/B][COLOR yellow]---[/COLOR]', 'viewpack')
			if SEPERATE == 'true':
				for name, version, url, url2, url3, gui, kodi, theme, icon, fanart, adult, description, preview in match:
					...
					addDir('[%s][B] %s [/B](v%s)' % (float(kodi), name, version), 'viewbuild', name, description=description, fanart=fanart,icon=icon, menu=menu, themeit=THEME2)
			else:
					...
					if SHOW15 == 'true':
						for name, version, url, url2, url3, gui, kodi, theme, icon, fanart, adult, description, preview in match:
							...
							if kodiv <= 15:
								menu = createMenu('install', '', name)
								addDir('[%s][B] %s [/B](v%s)' % (float(kodi), name, version), 'viewbuild', name, description=description, fanart=fanart,icon=icon, menu=menu, themeit=THEME2)
		else: addFile('Text file for builds not formated correctly.', '', icon=ICONCONTACT, themeit=THEME3)
	setView('files', 'viewType')
...
def viewBuild(name):
	...
	for version, url, url2, url3, gui, kodi, themefile, icon, fanart, adult, description, preview in match:
		...
		addFile('Build Information'     ,'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME1)
		if not preview == 'http://': addFile('Build Preview Video', 'buildpreview', name, description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def apkMenu():
	...
	#addDir ('Kodi'       ,'kodi1', icon=ICONSAVE,     themeit=THEME1)
	#addDir ('SPMC'       ,'spmc1', icon=ICONSAVE,     themeit=THEME1)
	...
	if HIDESPACERS == 'No': addFile(wiz.sep('Playonmonkey Apps List'), '', themeit=THEME3)
...
def saveMenu():
	...
	addDir ('Keep Addon Login Info'               ,'login',         icon=ICONLOGIN, themeit=THEME1)
	...
	addFile('Save  Addon Login Info: %s' % login.replace('true',on).replace('false',off)                  ,'togglesetting', 'keeplogin',      icon=ICONLOGIN, themeit=THEME1)
	if DEVELOPER == 'true': addFile('Keep \'Sources.xml\': %s' % sources.replace('true',on).replace('false',off)           ,'togglesetting', 'keepsources',    icon=ICONSAVE,  themeit=THEME1)
	if DEVELOPER == 'true': addFile('Keep \'Profiles.xml\': %s' % profiles.replace('true',on).replace('false',off)         ,'togglesetting', 'keepprofiles',   icon=ICONSAVE,  themeit=THEME1)
...
def buildWizard(name, type, theme=None):
	...
	if type == 'gui':
		...
		if yes_pressed:
			...
			if not buildzip1 == 'http://': buildzips.append("Free Server 1")
			if not buildzip2 == 'http://': buildzips.append("Premium Server 2")
			if not buildzip3 == 'http://': buildzips.append("Premium Server 3")
			if len(buildzips) > 1:
				select = DIALOG.select("Select the server to download the build from", buildzips)
				selected = buildzips[select]
				if select == -1 or selected == "Free Server 1":
					buildzip = buildzip1
				elif selected == "Premium Server 2":
					buildzip = buildzip2
				elif selected == "Premium Server 3":
					buildzip = buildzip3
			...
			if int(float(percent)) > 0:
				...
				if int(float(errors)) > 0:
					DIALOG.ok(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild( name,'version')), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'View them in the log viewer after reboot[/COLOR]')
...
################# Parse #################################
exec("import re;import base64");exec((lambda p,y:(lambda o,b,f:re.sub(o,b,f))(r"([0-9a-f]+)",lambda m:p(m,y),base64.b64decode("MjEoIjQgMTE7NCA1Iik7MjEoKDIzIDFiLDFjOigyMyAxZSxiLGY6MTEuMWQoMWUsYixmKSkoMjQiKFswLTIwLWZdKykiLDIzIDFmOjFiKDFmLDFjKSw1LjgoIjIyPT0iKSkpKDIzIGEsYjpiWzE3KCIxYSIrYS4xOCgxKSwxNildLCIyfDN8Nnw5fDd8MTR8Y3xlfGR8MTJ8MTN8MTV8MTkiLjEwKCJ8IikpKQ==")))(lambda a,b:b[int("0x"+a.group(1),16)],"0|1|Gorillawizard|executebuiltin|import|base64|speedtest|speedMenu|b64decode|Runscript|a|b|special|plugin|addons|f|split|re|home|xbmc|program|def|16|int|group|py|0x|p|y|sub|o|m|9a|exec|YiA0KCk6CglhLjEoJzMoIjY6Ly85LzcvOC41LjAvMi5jIiknKQ|lambda|r".split("|")))
